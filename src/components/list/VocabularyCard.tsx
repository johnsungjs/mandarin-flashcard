      ...
      <div className="flex h-full flex-col items-center justify-center text-center">
        ...
        <div
          className={`overflow-hidden transition-all duration-300 ${
            revealed ? "mt-2 max-h-32 opacity-100" : "mt-0 max-h-0 opacity-0"
          }`}
        >
          <div className="space-y-0 text-center leading-tight">
            <div className="text-sm text-slate-200">{pinyin}</div>

            <div className="text-sm text-slate-200">{meaning}</div>
          </div>
        </div>
      </div>
    </button>
